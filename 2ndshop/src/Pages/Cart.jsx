...
import { decQty, incQty, removeItem, clearItem } from "../redux/CartReducer/action";
...
const Cart = () => {
  ...
  let quantity = 0;
  var img = require(`../img/image/12369.jpg`);
  cart.forEach((item) => {
    show_price += convertStringIntoNumber(item.price) * item.qty;
    discount_price += convertStringIntoNumber(item.discount) * item.qty;
    quantity += item.qty;
    img = require(`../img/image/${item.image}`);
  });
